...
const createSchemas = async () => {
  const sqlDir = path.join(__dirname, '../../queries/DB');
  try {
    // USER_DB SQL 파일 실행
    await executeSqlFile(
      databaseManager.pools.USER_DB,
      path.join(sqlDir, 'user.db.sql'),
    );
    await executeSqlFile(
      databaseManager.pools.GAME_DB,
      path.join(sqlDir, 'game.db.sql'),
    );
